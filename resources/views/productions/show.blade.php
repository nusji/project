                            ...
                            @foreach ($production->productionDetails as $detail)
                                <div class="p-4 bg-gray-100 rounded-lg">
                                    <div class="flex items-center space-x-2">
                                        <span class="flex-grow font-semibold">{{ $detail->menu->menu_name }}</span>
                                        <span>{{ $detail->quantity }} กิโลกรัม</span>
                                    </div>
    
                                    <!-- ส่วนที่จะแสดงสูตรของแต่ละเมนูพร้อมการคำนวณ -->
                                    <div class="mt-4">
                                        <h4 class="font-medium">วัตถุดิบที่ใช้:</h4>
                                        <ul class="list-disc list-inside text-sm text-gray-800 mt-2">
                                            @foreach ($detail->menu->recipes as $recipe)
                                                <!-- คำนวณปริมาณวัตถุดิบตามจำนวนที่ผลิต -->
                                                @php
                                                    $ingredientRequired = $recipe->amount * $detail->quantity;
                                                @endphp
                                                <li>{{ $recipe->ingredient->ingredient_name }}: {{ $ingredientRequired }} {{ $recipe->ingredient->ingredient_unit }}</li>
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
