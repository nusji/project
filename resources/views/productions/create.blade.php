...
<div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <x-breadcrumb :paths="[['label' => 'ระบบการผลิต', 'url' => route('productions.index')], ['label' => 'สร้างรายการผลิตใหม่']]" />
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form action="{{ route('productions.store') }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label for="production_date" class="block text-sm font-medium text-gray-700 mb-2">วันที่ผลิต</label>
                        ...
                        <div id="menus-container" class="space-y-2">
                            @if(old('menus'))
                                ...
                                <div class="menu-row flex items-center space-x-2 p-2 bg-gray-100 rounded">
                                    ...
                                    <span class="flex-grow">{{ $menus->firstWhere('id', $menu['id'])->menu_name }}</span>
                                    <div class="flex items-center space-x-2">
                                        <input type="number" name="menus[{{ $index }}][quantity]" placeholder="จำนวนที่ผลิต" value="{{ $menu['quantity'] }}" 
                                               class="w-32 py-1 px-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displaySearchResults(results) {
        ...
        results.forEach(menu => {
            ...
            button.type = 'button';
            button.className = 'bg-gray-200 hover:bg-gray-300 text-gray-800 font-bold py-1 px-2 rounded mr-2';
