            ...
            <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    @if ($order->order_receipt)
                        <div class="mt-8">
                            ...
                            <img src="{{ asset('storage/' . $order->order_receipt) }}" alt="รูปโปรไฟล์"
                ...
                <div class="px-6 py-4 bg-gray-100 flex justify-between">
                    <a href="{{ route('orders.edit', $order) }}"
                        class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded transition duration-300">แก้ไข</a>
                    <form action="{{ route('orders.destroy', $order) }}" method="POST" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded transition duration-300"
                            onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบรายการนี้?')">ลบ</button>
                    </form>
                    <a href="{{ route('orders.index') }}"
                        class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition duration-300">กลับไปหน้ารายการ</a>
                </div>
            </div>
        </div>
    </div>
