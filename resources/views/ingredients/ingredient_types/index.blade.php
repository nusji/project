    ...
    <div class="container mx-auto px-4 py-8">
        <div class="bg-white shadow-lg rounded-lg overflow-hidden">
            <div class="p-6 bg-gray-50 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">ประเภทวัตถุดิบ</h2>

                <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 md:space-x-4">
                    <div class="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                        <a href="{{ route('ingredient_types.create') }}"
                            class="inline-flex items-center px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition duration-300 ease-in-out shadow-md">
                        ...
                        <a href="{{ route('ingredients.index') }}"
                            class="inline-flex items-center bg-gray-500 hover:bg-gray-600 text-white font-medium py-2 px-4 rounded text-sm transition duration-300 ease-in-out">
        ...
        @foreach ($ingredientTypes as $ingredientType)
            <form id="delete-form-{{ $ingredientType->id }}"
                action="{{ route('ingredient_types.destroy', $ingredientType->id) }}" method="POST"
                style="display: none;">
                @csrf
                @method('DELETE')
            </form>
