...
@section('content')
    <div class="container mx-auto px-4 py-0">
        ...
        @if (auth()->user()->role === 'owner')
            ...
            <div class="text-sm font-md text-gray-800 mb-2">
                <form method="GET" action="{{ route('payrolls.index') }}" class="text-right">
                    ...
                    <select name="month" id="month" class="mx-2">
                        @for ($i = 1; $i <= 12; $i++)
                    ...
                    <select name="year" id="year" class="mx-2">
        ...
        @if (auth()->user()->role === 'employee')
            <h2 class="text-2xl font-bold text-gray-800 mb-2">ประวัติรับเงินเดือน</h2>
        @endif
    </div>
    ...
    <div class="container mx-auto px-4 py-6">
        <div class="p-6">
            ...
            @elseif(auth()->user()->role === 'employee')
                <h1 class="text-xl font-bold text-gray-800 mb-4">ประวัติการจ่ายเงินเดือนของคุณ</h1>
            @endif
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @php
                            // สำหรับการกรองข้อมูลของ employee
                            $currentUserRole = auth()->user()->role;
                            $currentEmployeeId = $currentUserRole === 'employee' ? auth()->user()->employee->id : null;
                            $filteredPayrolls = $currentUserRole === 'employee'
                                ? $payrolls->filter(function($payroll) use ($currentEmployeeId) {
                                    return $payroll->employee_id === $currentEmployeeId;
                                })
                                : $payrolls;
                        @endphp
                        ...
                        @if ($filteredPayrolls->isEmpty())
                            <tr>
                                <td colspan="{{ auth()->user()->role === 'owner' ? 7 : 6 }}" class="px-6 py-4 text-center text-gray-500">
                                    @if (auth()->user()->role === 'owner')
                                        ไม่มีข้อมูลพนักงานในระบบ
                                    @elseif(auth()->user()->role === 'employee')
                                        ไม่มีข้อมูลการจ่ายเงินเดือนของคุณในระบบ
                                    @endif
                                </td>
                            </tr>
                        @else
                            @foreach ($filteredPayrolls as $payroll)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ number_format($payroll->employee->salary, 2) }} บาท
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ number_format($payroll->bonus, 2) }} บาท
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ number_format($payroll->deductions, 2) }} บาท
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        {{ number_format($payroll->net_salary, 2) }} บาท
                                    </td>
                                    @if (auth()->user()->role === 'owner')
                                        <td class="px-6 py-4 whitespace-nowrap">
